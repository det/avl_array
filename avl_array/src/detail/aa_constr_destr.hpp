...
inline
  avl_array<T,A,bW,W,bP,P>::avl_array ()
{
  init ();
}
...
// Sequence [from,to) constructor: create an avl_array with an
...
template <class IT>
inline
  avl_array<T,A,bW,W,bP,P>::avl_array (IT from, IT to)
{
  ...
  avl_array<T,A,bW,W,bP,P>::init ()
{
  ...
  if (bW)
    *node_t::m_node_width = W(0);             // Zero width
